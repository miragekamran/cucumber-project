...
// subfleet types
And(/^User selects a Subfleet Type$/, () => {
    cy.get(airportAuthorization.subfleetType()).type('A-988{enter}')
})

And(/^User adds authorization$/, () => {
    cy.contains(airportAuthorization.addAuthBtn(), " Add Authorizations ").click()
})
